    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-4" id="forgot-form">
      ...
      <div class="rounded-xl border border-border bg-background/40 p-4 text-sm">
        ...
        <div class="text-xs text-muted-foreground mb-3">
          If you didn’t receive the email, you can contact support or track your request.
        </div>
...
<script>
  ...
  const config = {
    alumni:  { label: 'Academic ID',   placeholder: 'Enter your academic ID', type: 'text'  },
    college: { label: 'Email Address', placeholder: 'Enter college email',    type: 'email' },
    company: { label: 'Email Address', placeholder: 'Enter company email',    type: 'email' },
    admin:   { label: 'Email Address', placeholder: 'Enter admin email',      type: 'email' },
  };
  ...
  function copyResetLink(text) {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard.writeText(text).then(() => showMiniToast('Copied')).catch(() => {});
      return;
    }
    ...
    ta.select();
    try { document.execCommand('copy'); showMiniToast('Copied'); } catch (e) {}
    ta.remove();
  }
</script>
@endsection
