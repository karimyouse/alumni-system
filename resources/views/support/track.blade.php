...
<div class="min-h-screen relative overflow-hidden flex items-center justify-center">
  ...
  <div class="relative z-10 w-full max-w-3xl rounded-xl border border-border bg-card/80 backdrop-blur p-6 shadow-xl">
    <div class="flex flex-col items-center text-center mb-6">
      ...
      <h1 class="text-2xl font-bold">Track Support Request</h1>
      <p class="text-sm text-muted-foreground">Enter your tracking code and email to view the ticket</p>
    </div>
    ...
    @if(!empty($notFound))
      <div class="mb-5 rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-sm">
        ...
        <div class="text-destructive/90">
          We couldn't find a ticket for this tracking code and email.
        </div>
      </div>
    @endif
      ...
      <div class="mt-6 space-y-4">
        <div class="rounded-xl border border-border p-4">
          ...
          <div class="text-xs text-muted-foreground mt-2">
            Tracking Code: <span class="font-mono text-foreground">{{ $ticket->tracking_code }}</span>
          </div>
        ...
        <div class="rounded-xl border border-border p-4">
          <div class="text-sm font-semibold mb-1">{{ $ticket->title ?? $ticket->subject ?? 'Request' }}</div>
        ...
        <div class="rounded-xl border border-border p-4 bg-accent/10">
          ...
          @else
            <div class="text-sm text-muted-foreground">No reply yet. Please check again later.</div>
          @endif
        </div>
      </div>
    @endif
